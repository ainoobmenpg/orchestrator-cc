# Orchestrator CC - Issues Index

このディレクトリには、orchestrator-ccプロジェクトのIssue（GitHub削除後にローカル管理）が保存されています。
...
- **ローカル運用**: GitHubアカウント削除後はローカルでのIssue管理となります
- **リポジトリ削除禁止**: ローカルリポジトリの削除は絶対に禁止されています
- **コミット**: Issueの変更や進捗状況は適宜Gitコミットしてください
