...
- タイムアウトは30秒とします
- メンバーが応答しない場合でも、タイムアウト後に次のメンバーへ進んでください
- すべてのメンバーの処理（応答またはタイムアウト）完了後、ステップ4へ進みます

### ステップ4: TeamDelete でチームを削除（必須）
**最も重要**: TeamDeleteツールを使用してチームを削除します。
- これはTeam Leadが直接呼び出すClaude Codeのツールです
- Pythonコードから呼び出す必要はありません
- すべてのメンバーの処理完了後、必ず実行してください

```
TeamDelete {}
```

**重要**: TeamDeleteを呼び出さないと、チームディレクトリとタスクディレクトリが残り続け、次の `/team` コマンドが正常に動作しなくなります。
