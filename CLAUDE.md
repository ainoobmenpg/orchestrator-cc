...
### ルール0: Agent Teamsの活用（推奨）

**このプロジェクトでは、タスクの複雑度に応じてAgent Teams機能を活用してください。**

タスクの複雑度に応じて、以下のガイドラインを参考にしてください：
...
| **並列作業の活用** | 独立したタスクは並列実行で効率化 |
| **依存関係の明示** | タスク間の依存はTaskUpdateのblocks/blockedByで管理 |
...
**タスク分割のガイドライン:**
1. **依存関係を特定** - どのタスクが他のタスクの結果に依存するか
2. **独立タスクを抽出** - 並列実行可能なタスクを分離
3. **適切な粒度** - タスクが大きすぎず小さすぎないサイズに分割
4. **結果の統合** - 並列タスクの結果を統合する計画を立案

**例: 並列タスク割り当て**
```
タスク: 複数モジュールのリファクタリング
├── スペシャリストA: モジュールXのリファクタリング（独立）
├── スペシャリストB: モジュールYのリファクタリング（独立）
└── スペシャリストC: 共有インターフェースの更新（X,Y完了後に実行）
```
