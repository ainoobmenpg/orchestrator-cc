...
バックグラウンドで継続的に実行する必要があるプロセスの場合：

- **tmuxセッションを使用する**（バックグラウンドモード `&` は避ける）
- `tmux new-session -d -s <name>` でデタッチドセッションを作成
- 必要に応じて `tmux attach -t <name>` でアタッチ
- 理由：バックグラウンドモードはすぐに終了するため
